...
class Controller:
    ...
    def go(self, steps=1):
        ...
        for _ in range(steps):
            ...
            self.anim.smooth_bar3D(counts=counts, 
    ...
    def counts_barPlot(self):
        
        counts = np.zeros((self.xsize, self.ysize))
            
        for i in range(self.xsize):
            for j in range(self.ysize):
                counts[i][j] = self.grid.cells[i][j].size
...
if __name__ == '__main__':
    ...
    for i in range(100):
        print(i)
        controller = Controller(1000, 50, 50, 100, draw_step=1)
        controller.go(350)
        for _ in range(35):
